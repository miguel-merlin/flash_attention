...
def main() -> None:
    ...
    print(f"\nMedian latency / forward: {format_time_us(median_s)}")
    print(f"Forward calls / sec:      {calls_per_s:,.1f}")
    print(f"Batch query rows / sec:   {query_rows_per_s:,.1f}  (B={B} queries timed together per call)")
    print(

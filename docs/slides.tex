...
\begin{frame}{Paged Attention: Kernel Design (V1 vs V2)}
    ...
    \begin{tikzpicture}[
        ...
        stepbox/.style={font=\fontsize{6.7}{7.7}\selectfont, align=left},
        arrow/.style={-{Stealth[length=5pt]}, thick}
    ]
        % V1
        \node[box, fill=red!5] (v1) at (-3.45, 0) {};
        \node[font=\scriptsize\bfseries, color=red!60!black] at (-3.45, 1.95)
            {V1: Scalar-Oriented Naive Kernel};

        \node[stepbox] at (-3.45, 0.05) {
            \textbf{Mapping:} 1 thread $\to$ 1 scalar\\
            \quad computes $out[b,h,x]$\\[3pt]

            For each output dim $x$:\\
            \quad scan all $N$ cached tokens\\
            \quad page lookup: logical $\to$ physical\\
            \quad compute $q \cdot k_t$\\
            \quad find softmax max\\
            \quad recompute $q \cdot k_t$\\
            \quad compute sum-exp + weighted value\\[3pt]

            \textcolor{red!70!black}{Problem: scores, max, sum-exp}\\
            \textcolor{red!70!black}{are recomputed for every $x$}
        };

        % Arrow
        \draw[arrow, very thick, green!50!black] (-0.85, 0) -- (0.85, 0);
        \node[font=\fontsize{5}{6}\selectfont, color=green!50!black, align=center] at (0, 0.55)
    {$12.8\times$ speedup\\by sharing attention work};

        % V2
        \node[box, fill=green!5] (v2) at (3.45, 0) {};
        \node[font=\scriptsize\bfseries, color=green!50!black] at (3.45, 1.95)
            {V2: Vector-Oriented Cooperative Kernel};

        \node[stepbox] at (3.45, 0.05) {
            \textbf{Mapping:} 1 block $\to$ 1 vector\\
            \quad computes $out[b,h,:]$\\[3pt]

            Within each $(b,h)$ block:\\
            \quad load $q[b,h,:]$ into shared mem\\
            \quad compute scores $q \cdot k_t$ once\\
            \quad reduce local max $\to$ max score\\
            \quad compute $e^{score - max}$\\
            \quad reduce local sums $\to$ sum-exp\\
            \quad split output dims $x$ across threads\\[3pt]

            \textcolor{green!50!black}{Scores/weights computed once}\\
            \textcolor{green!50!black}{and reused across all $d$ dims}
        };
    ...
    V2 changes the decomposition from \emph{one scalar per thread} to
    \emph{one output vector per block}. This lets the block reuse the query vector,
    attention scores, softmax max, and softmax denominator across all $d$ output dimensions.
\end{frame}
% ===== SLIDE 9: Paged Attention -- Runtime Scaling =====
\begin{frame}{Paged Attention: Runtime Scaling}
    \begin{table}[]
    \centering
    \resizebox{0.78\columnwidth}{!}{%
    \begin{tabular}{|c|c|c|c|}
    \hline
    \textbf{Batch Size} & \textbf{Paged v1} & \textbf{Paged v2} & \textbf{Speedup} \\ \hline
    B=1  & 9.45 ms  & 955.4 $\mu$s & 9.9$\times$  \\
    B=2  & 9.50 ms  & 970.4 $\mu$s & 9.8$\times$  \\
    B=4  & 9.73 ms  & 974.0 $\mu$s & 10.0$\times$ \\
    B=8  & 9.96 ms  & 974.1 $\mu$s & 10.2$\times$ \\
    B=16 & 10.06 ms & 1.09 ms       & 9.2$\times$  \\
    B=32 & 10.05 ms & 1.21 ms       & 8.3$\times$  \\ \hline
    \end{tabular}%
    }
    \caption{Paged Attention runtime on CUDA while scaling batch size at fixed $H=8$, $N=2048$, $d=64$.}
    \end{table}

    \vspace{0.1cm}
    \footnotesize
    \textbf{Takeaway:} Paged v2 is consistently $\sim$8--10$\times$ faster than v1.
    V1 computes one output scalar per thread and repeatedly recomputes scores/softmax statistics.
    V2 computes one output vector per block, reusing scores, max, and denominator across all $d$ dimensions.
\end{frame}

% ===== SLIDE 10: Paged Attention -- Memory Scaling =====
\begin{frame}{Paged Attention: Memory Scaling on A100}
\vspace{-0.25cm}

\begin{center}
\footnotesize
\textbf{Peak GPU memory while scaling batch size at fixed $H=8$, $N=2048$, $d=64$}
\end{center}
...
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Batch} & \textbf{Vanilla} & \textbf{FlashCUDA} & \textbf{Paged Attention} \\ \hline
B=1   & 284 MB  & 28 MB  & 28 MB  \\
B=8   & 2.0 GB  & 144 MB & 112 MB \\
B=32  & 8.0 GB  & 528 MB & 400 MB \\
B=64  & 17.0 GB & 1.0 GB & 784 MB \\
B=128 & 34.0 GB & 2.0 GB & 1.0 GB \\ \hline
\end{tabular}%
}
\end{center}
...
\footnotesize
\textbf{Takeaway:} Vanilla attention reaches $34$ GB at $B=128$, while Paged Attention stays near $1$ GB by avoiding full attention-matrix materialization and using a paged KV cache.

\end{frame}

% ===== SLIDE 11: GPT-2 End-to-End Integration =====

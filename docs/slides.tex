...
\begin{frame}{Problem: Attention Computation}
    \begin{itemize}
        ...
        \item It is \textbf{memory bandwidth-bound}, not compute-bound.
        \item The attention matrix $S = QK^T$ has size $O(N^2)$, where $N$ is the sequence length.
        \item Moving $O(N^2)$ intermediate data to/from \textbf{Global Memory (HBM)} causes a severe bottleneck.
    \end{itemize}
\end{frame}
...
\begin{frame}{Flash Attention: Results}
    ...
    \end{table}
    \begin{itemize}
        \item \textbf{Memory:} At $N{=}1024$, flash uses $\mathbf{7{\times}}$ less memory (42\,MB vs 298\,MB), confirming the $O(N)$ profile.
        \item \textbf{Latency:} Our pedagogical kernel is slower due to un-optimized shared-memory tiling, but the \emph{memory savings are the primary goal} of this implementation.
    \end{itemize}
\end{frame}
